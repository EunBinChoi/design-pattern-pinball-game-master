...
public class TornadoMng : MonoBehaviour
{
		...
		void FixedUpdate ()
		{
				if (bInside) {
						...
						dir = this.transform.position - Ball._instance.transform.position;
						//distance = Vector3.Distance (this.transform.position, gameBall.transform.position);
						if (Time.time >= endTime) {
                                Ball._instance.GetComponent<Rigidbody>().AddForce (dir * (Random.Range (min_grv* duration, max_grv * duration)), ForceMode.Acceleration);
								bInside = false;
						} else {
                                Ball._instance.GetComponent<Rigidbody>().AddForce (dir * (gravity), ForceMode.Acceleration);
						}
						//print (distance);
						gravity+=3;
		...
		{
				if (other.gameObject.tag == Ball._instance.tag) {	//그 충돌된 물체가 Ball 이면 
		...
		void OnTriggerExit (Collider other)
		{
				...
				if (other.gameObject.tag == Ball._instance.tag) {   //볼이 밖으로 빠져 나갔다면?
